...
void engine_marktasks_mapper(void *map_data, int num_elements,
  ...
  for (int ind = 0; ind < num_elements; ind++) {
    ...
    if (t->ci != NULL)
      celltrace(t->ci, "MARKING ci %s/%s ACTIVE? %d", taskID_names[t->type], subtaskID_names[t->subtype], cell_is_rt_active(t->ci, e));
    if (t->cj != NULL)
      celltrace(t->cj, "MARKING cj.%s/%s ACTIVE? %d", taskID_names[t->type], subtaskID_names[t->subtype], cell_is_rt_active(t->cj, e));
    if (t->type == task_type_rt_advance_cell_time)
      celltrace(t->ci, "MARKING ci. ACTIVE? %d", cell_is_rt_active(t->ci, e));
