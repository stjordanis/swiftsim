...
/**
 * @brief Intermediate task after the matrix loop that does final operations
 * on the matrix quantities and optionally slope limits the gradients
 *
 * @param r The runner thread.
 * @param c The cell.
 * @param timer Are we timing this ?
...
/* matrix loop */
void runner_do_matrix_ghost(struct runner *r, struct cell *c, int timer) {
  ...
  if (c->split) {
    for (int k = 0; k < 8; k++)
      if (c->progeny[k] != NULL) runner_do_matrix_ghost(r, c->progeny[k], 0);
  } else {
  ...
  if (timer) TIMER_TOC(timer_do_matrix_ghost);
