...
void engine_addtasks_send_hydro(struct engine *e, struct cell *ci,
  ...
  if (l != NULL) {
    ...
    if (t_xv == NULL) {
      ...
      if (with_rt){
        /* Add the RT sends */
        t_rt_gradient = scheduler_addtask(s, task_type_send, task_subtype_rt_gradient, ci->mpi.tag, 0, ci, cj);

        t_rt_transport = scheduler_addtask(s, task_type_send, task_subtype_rt_transport, ci->mpi.tag, 0, ci, cj);
      }
      ...
      if (with_rt){ 
        ...
        /* Safety measure: collect dependencies and make sure data is sent before modifying it */
        scheduler_addunlock(s, t_rt_gradient, ci->hydro.super->rt.rt_ghost2);

        /* Safety measure: collect dependencies and make sure data is sent before modifying it */
        scheduler_addunlock(s, t_rt_transport, ci->hydro.super->rt.rt_transport_out);
    ...
    for (int k = 0; k < 8; k++)
      if (ci->progeny[k] != NULL)
        engine_addtasks_send_hydro(
            ...
            t_pack_limiter, t_rt_gradient, t_rt_transport, with_feedback, with_limiter, with_sync, with_rt);
    ...
    if (t_rt_gradient == NULL) {
      ...
      for (struct link *l2 = ci->mpi.send; l2 != NULL; l2 = l2->next) {
        if ((l2->t->subtype == task_subtype_xv) &&
            (l2->t->ci->nodeID == nodeID ||
             (l2->t->cj != NULL && l2->t->cj->nodeID == nodeID))) {
          t_xv = l2->t;
          break;
        }
      }
...
void engine_addtasks_recv_hydro(
    struct engine *e, struct cell *c, struct task *t_xv, struct task *t_rho, struct task *t_gradient, struct task *t_prep1, struct task *t_limiter, struct task *t_unpack_limiter, struct task *t_rt_gradient, struct task *t_rt_transport, struct task *t_rt_advance_cell_time, struct task *t_rt_sorts, struct task *const tend, const int with_feedback, const int with_black_holes, const int with_limiter, const int with_sync, const int with_rt) {
  ...
  if (t_xv == NULL && c->hydro.density != NULL) {
    ...
    if (with_rt){
      ...
      t_rt_advance_cell_time =
          scheduler_addtask(s, task_type_rt_advance_cell_time, task_subtype_none, 0, 0, c->super, NULL);
    ...
    for (int k = 0; k < 8; k++)
      if (c->progeny[k] != NULL)
        engine_addtasks_recv_hydro(e, c->progeny[k], t_xv, t_rho, t_gradient,
                                   ...
                                   t_rt_gradient, t_rt_transport, t_rt_advance_cell_time, t_rt_sorts, tend,
...
void engine_addtasks_send_mapper(void *map_data, int num_elements,
  ...
  for (int k = 0; k < num_elements; k++) {
    ...
    if (!cell_is_empty(ci)) {
      ...
      struct task *tend = scheduler_addtask(&e->sched, task_type_send, task_subtype_tend,
                               ci->mpi.tag, 0, ci, cj);
    ...
    if ((e->policy & engine_policy_hydro) && (type & proxy_cell_type_hydro))
      engine_addtasks_send_hydro(e, ci, cj, /*t_xv=*/NULL,
                                 ...
                                 with_feedback, with_limiter, with_sync, with_rt);
...
void engine_addtasks_recv_mapper(void *map_data, int num_elements,
  ...
  for (int k = 0; k < num_elements; k++) {
     ...
     * connection. */
    if ((e->policy & engine_policy_hydro) && (type & proxy_cell_type_hydro)) {
      engine_addtasks_recv_hydro(e, ci, /*t_xv=*/NULL, /*t_rho=*/NULL, /*t_gradient=*/NULL, /*t_prep1=*/NULL, /*t_limiter=*/NULL, /*t_unpack_limiter=*/NULL, /*t_rt_gradient=*/NULL, /*t_rt_transport=*/NULL, /*t_rt_advance_cell_time=*/NULL, /*t_rt_sorts=*/NULL, tend, with_feedback, with_black_holes, with_limiter, with_sync, with_rt);
    }
